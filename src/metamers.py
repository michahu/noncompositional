...
def build_ngram_model(corpus, n, tokenizer_name="bert-base-uncased"):
    ...
    token_ids = []
    for x, y in corpus:
        input_str = tokenizer.encode(x, add_special_tokens=False)[-n+1:] + tokenizer.encode(y, add_special_tokens=False)
        # if 4255 in input_str:
        #     breakpoint()
...
def main(train_file, out_file, n=2):
    ...
    ngram_matrix, vocab_size, tokenizer = build_ngram_model(corpus, n, tokenizer_name='openai-community/gpt2')

    def make_ngram_metamer(example):
        encoded_en = tokenizer.encode(example['translation']['en'], add_special_tokens=False)
        seed_len = len(tokenizer.encode(example['translation']['mentalese'], add_special_tokens=False))
